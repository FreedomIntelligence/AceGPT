...
class ArabicCultureBenchmark(BenchmarkBase):
    ...
    def _init_task_prompts(self):
        ...
        self.task_prompt_zero_shot = {
            task: (
          'أنت مساعد ملم بالثقافة والقيم العربية المحلية. توجد سؤال أو عبارة أدناه. الرجاء الإجابة بـ "نعم" أو "لا" من وجهة نظر الثقافة والقيم في العالم العربي أو التحقق من الحقائق لتعكس إدراكك للقيمة/الحقيقة للسؤال أو العبارة. الإجابة بـ "نعم" تعني أن إجابة السؤال هي "نعم" أو أن العبارة تتوافق مع إدراكك للحقيقة/الثقافة. في المقابل، الإجابة بـ "لا" تعني أن إجابة السؤال هي "لا" أو أن العبارة لا تتوافق مع إدراكك للحقيقة/الثقافة.' + '\n\n{input}\n' + 'الرجاء إخراج "نعم" أو "لا" دون شرح.' 
            )
